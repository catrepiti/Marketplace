...
import {
  ...
  ChevronRight, Wifi, WifiOff, User, CreditCard, Link2,
  Settings, AlertCircle, Clock, Zap, ExternalLink,
} from 'lucide-react'
                    ...
                    <div className="p-6">
                      <div className="flex items-start justify-between mb-6">
                        ...
                        <div className="text-right">
                          ...
                          <p className="text-[11px] text-white/20">/{plan.interval === 'anual' ? 'mês (anual)' : plan.interval === 'semestral' ? 'mês (semestral)' : 'mês'}</p>
                        </div>
                      </div>
              ...
              <div className="space-y-6">
                ...
                <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-5">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span className="text-xs font-mono text-white/40">{connectedCount}/{plan?.maxAccounts ?? 3}</span>
                  </div>
                  <div className="h-2 bg-white/5 rounded-full overflow-hidden">
                    <div className="h-full bg-primary rounded-full transition-all duration-500"
                      style={{ width: `${(connectedCount / (plan?.maxAccounts ?? 3)) * 100}%` }} />
                  </div>
                  <p className="text-[11px] text-white/20 mt-2">
                    Sua licença permite vincular até {plan?.maxAccounts ?? 3} contas de marketplace
                  </p>
                </div>
                ...
                <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] overflow-hidden">
                  ...
                  {accounts.length > 0 ? (
                            ...
                            <div className="flex items-center gap-3">
                              {a.connected ? (
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  <span className="text-xs font-medium text-white/25">Desconectado</span>
                                </div>
                              )}
                            </div>
                          </div>
                    ...
                    <div className="p-6 text-center py-12">
                      ...
                      <p className="text-xs text-white/15">Conecte seus marketplaces para começar a ver dados em tempo real</p>
                    </div>
                  )}
                </div>
                ...
                <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] overflow-hidden">
                  ...
                  <div className="divide-y divide-white/[0.04]">
                    {Object.entries(MP_META).map(([key, meta]) => {
                      const isConnected = accounts.some(a => a.marketplace === key && a.connected)
                      return (
                        <div key={key} className="px-6 py-4 flex items-center justify-between">
                          ...
                          {isConnected ? (
                            ...
                            <button className="text-xs font-medium text-primary bg-primary/5 border border-primary/20 px-3 py-1.5 rounded-lg hover:bg-primary/10 transition-colors flex items-center gap-1.5">
